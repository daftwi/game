...
func _ready():
	...
	if !PlayerVariables.pay["ThirdTask"] and !PlayerVariables.tasks_completed["ThirdTask"]:
		...
		$Bugg4/Sprite.visible = true
	if PlayerVariables.buggs_found["Bathroom"]:
		$Bugg/Sprite.visible = false
		$Bugg2/Sprite.visible = false
		$Bugg3/Sprite.visible = false
		$Bugg4/Sprite.visible = false
...
func _on_Bugg_input_event(viewport, event, shape_idx):
	if event is InputEventMouseButton:
		if event.is_pressed():
			...
			c += 1
			if c == 4:
				PlayerVariables.buggs_found["Bathroom"] = true
...
func _on_Bugg4_input_event(viewport, event, shape_idx):
	if event is InputEventMouseButton:
		if event.is_pressed():
			PlayerVariables.buggs -= 1
			$Bugg4/Sprite.visible = false
